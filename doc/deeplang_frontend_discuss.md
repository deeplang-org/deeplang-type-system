...
练琪灏：1、变量名覆盖函数名。2、函数体返回值类型匹配，stmt返回。3、函数decl和方法的decl，在符号表的中ID。4、hash table imuttable问题。5、不做 隐式类型转换，数值暂时也不隐式转换。6、委托的检查的讨论。7、模式匹配的检查。

刘请：重名的讨论。

陈亦棠：修改deeplang.cf。函数不是一等公民，结构体和ADT。模式匹配，控制语句。Interface的语法变化比较大。

### 2022.3.13例会纪要

练琪灏：报错信息，后面统一文档定义错误信息。

刘请：测试用例，UT（单元测试），LLT。ast 形式的用例。功能性用例。内存管理系统设计。
